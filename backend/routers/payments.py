...
class VerifyPaymentRequest(BaseModel):
    ...
    tx_ref: str
    plan_id: str  # Changed from 'plan' to match frontend
...
async def checkout_page(
    plan_id: str,      # NEW: Accept plan_id instead of amount/public_key
    email: str,
    name: str,
    tx_ref: Optional[str] = None,
    currency: str = "USD",
    request: Request = None,  # To build callback URL dynamically
):
    """
    Checkout page - accepts plan_id, looks up details automatically.
    No need to send amount or public_key from frontend.
    """
    ...
    # Build callback URL from request (dynamic)
...
async def verify_payment(
    ...
    try:
        ...
        # Only set subscription fields if they exist in your User model
        ...
        if hasattr(current_user, 'subscription_id'):
            current_user.subscription_id = transaction_id
        
        await db.commit()  # CRITICAL: Use commit, not flush!
...
async def flutterwave_webhook(
    ...
    if event == "charge.completed":
        ...
        logger.info(f"Payment completed: {tx_ref}, status: {tx_status}")
        # TODO: Update database if not already done via verify endpoint
