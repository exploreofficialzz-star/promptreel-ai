...
class _SplashScreenState extends ConsumerState<SplashScreen>
  ...
  bool _showContent = false; // NEW: Control when to show content
  ...
  void initState() {
    ...
    // Show content after short delay (ensures widget is built)
  ...
  Future<void> _init() async {
    // Web goes straight to login
    if (kIsWeb) {
      if (mounted) context.go('/login');
      return;
    }

    // Initialize ads in background (don't block)
    unawaited(Future(() => AdService.instance.initialize()));
    ...
    try {
      final isLoggedIn = await ref
          ...
          .isLoggedIn()
          .timeout(const Duration(seconds: 5)); // Reduced from 8s
  ...
  Widget build(BuildContext context) {
    ...
    // Mobile version - SIMPLIFIED to ensure it shows
    return Scaffold(
      ...
      children: [
        ...
        Column(
          children: [
            ...
            // ── App Icon ─────────────────────────────────────────
            // SIMPLIFIED: Use Icon instead of Image to avoid asset issues
            Container(
